...
import TrackYourShip from "../components/TrackYourShip";
export default function Navber() {
  ...
  const [currentPage, setcurrentbPage] = useState(
  ...
  useEffect(() => {
    setcurrentbPage(location.pathname.split("/")[1]);
  });
  useEffect(() => {
    return () => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <TrackYourShip />
          <NavTitle to={"/login"} active={currentPage == "login"}>
            {NavTitles.login}
          </NavTitle>
        </Grid>
        ...
        >
          <NavTitle to={"/salesCall"} active={currentPage == "salesCall"}>
            {NavTitles.sales}
          </NavTitle>
          <NavTitle to={"/prices"} active={currentPage == "prices"}>
            {NavTitles.prices}
          </NavTitle>
...
function NavTitle({ children, active, to, handleNavToggle }) {
  return (
    ...
    >
      <Link to={to} style={{ textDecoration: "none", fontFamily: "cairo" }}>
        <Box
          sx={{
            display: "flex",
            flexWrap: "nowrap",
